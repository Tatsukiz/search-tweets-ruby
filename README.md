...
+ Configure the client.
 + Specify the search API to request from, provide API credentials, and set app options. These are stored in a configuration YAML file. The client deafults to a ```./config/config.yaml``` file, but you can specify any path using the ```-c``` command-line option.
